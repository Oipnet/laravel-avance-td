...
Route::middleware('auth')->get('/avatar', function(Request $request) {
    return response()->view('avatar');
})->name('avatar');
Route::middleware('auth')->post('/avatar', function (Request $request) {
    $request->file('avatar')->storeAs('avatar/'. $request->user()->id, 'avatar.jpeg', 'public');
})->name('avatar_upload');
